...
ui <- tagList(
  ...
  navbarPage(
    ...
    # Panel de Análisis
    tabPanel("Análisis de interés 💡️",
             sidebarLayout(
               sidebarPanel(
                 selectInput("main_view_analysis", "Seleccionar vista:",
                             choices = c("Análisis Cluster Jerárquico 💠️" = "cluster",
                                         "Análisis de Correspondencia 👥" = "corresp"),
                             selected = "cluster"),
                 
                 conditionalPanel(
                   condition = "input.main_view_analysis == 'cluster'",
                   checkboxGroupInput("plot_choices", "Selecciona las opciones para ver:",
                                      choices = c("Dendrograma", "Silueta", "Tabla"),
                                      selected = c("Dendrograma", "Silueta", "Tabla")),
                   sliderInput("k", "Número de grupos (k):", min = 2, max = 4, value = 3),
                 ...
                 conditionalPanel(
                   ...
                   radioButtons("var1", "Elige Variable 1:",
                                choices = c("VEHICLE_1", "CAUSE", "HOUR", "DAY_OF_WEEK"), selected = "VEHICLE_1"),
                   radioButtons("var2", "Elige Variable 2:",
                                choices = c("VEHICLE_1", "CAUSE", "HOUR", "DAY_OF_WEEK"), selected = "CAUSE")
                 )
               ),
               ...
               mainPanel(
                 conditionalPanel(
                   condition = "input.main_view_analysis == 'cluster'",
                   conditionalPanel(condition = "input.plot_choices.indexOf('Dendrograma') > -1",
                                    plotOutput("dendrogramPlot", height = "500px")),
                   conditionalPanel(condition = "input.plot_choices.indexOf('Silueta') > -1",
                                    plotOutput("silhouettePlot")),
                   conditionalPanel(condition = "input.plot_choices.indexOf('Tabla') > -1",
