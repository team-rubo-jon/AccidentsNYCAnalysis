...
Vamos a hacer una representación dual, en la que combinamos el gráfico de barras y el gráfico de líneas, para mostrar la frecuencia de accidentes por día, mes y año. Para ello, vamos a crear una nueva variable que contenga la fecha en formato de fecha y otra que contenga el mes y año.
...
# Frecuendia de datos diarios
data_time |> 
  group_by(DATE) |>
  summarise(FREQ = sum(FREQ_DAY, na.rm = TRUE), .groups = "drop") |>
    ...
    labs(title = "Frecuencia de Accidentes por Día", x = "Fecha", y = "Frecuencia") +
    theme_nyc()
...
```
También, vamos a representar la frecuencia total de cada mes.
...
data_time |> 
  ...
  ggplot(aes(x = MONTH, y = FREQ_MONTH)) + 
    geom_histogram(stat = "identity", fill = "#1F3B73") +
    labs(title = "Frecuencia de Accidentes por Mes", x = "Fecha", y = "Frecuencia") +
    theme_nyc()
```
...
Por otro lado, en el histograma se puede ver como los meses de verano es donde mas accidentes se producen. Esto puede ser debido a que en verano la gente sale más a la calle, y por lo tanto, hay más accidentes, miestras que en invierno la gente tiende a quedarse más en casa.
...
Ahora vamos a crear una serie temporal por cada distrito, de tal forma que podamos comparar entre ellos según la fecha.
