...
```{r}
data_cleaned <- data |> 
  filter(if_any(LOCATION, ~ . != ""), LOCATION != '(0.0, 0.0)') |> 
  select(-ZIP.CODE, -LATITUDE, -LONGITUDE, -ON.STREET.NAME, -OFF.STREET.NAME, -CROSS.STREET.NAME, -VEHICLE.TYPE.CODE.3, -VEHICLE.TYPE.CODE.4, -VEHICLE.TYPE.CODE.5, -COLLISION_ID, -CONTRIBUTING.FACTOR.VEHICLE.5, -CONTRIBUTING.FACTOR.VEHICLE.4, -CONTRIBUTING.FACTOR.VEHICLE.3)
```
Breve justificación de la eliminación de cada variable
...
Una vez eliminados los individuos que no son de interes, seguimos teniendo demasiadas observaciones, por lo que para trabajar con los datos de forma más comoda hemos decidido hacer un muestreo de 20.000 observaciones
...
Una vez obtenido el dataset con el que trabajremos, vamos a visualizar las diferentes variables y sus comportamientos para tratar de entender mejor con que datos estamos trabajando antes de aplicar las diferentes técnicas

(Aqui añadir histogramas, matrices de correlacion de las variables, distribuciones de cada variable (violines)...)
