...
Para el caso de latitude y longitude, vamos a mostrar un histograma de la localización de los accidentes en el mapa de Nueva York. Para ello, vamos a usar la librería leaflet, que nos permite mostrar mapas interactivos. --> *DESCONOZCO PORQUÉ NO ME FUNCIONA, PROBAR A VER SI A VOSOTROS OS DEJA PORFA*
...
datos_resumen <- data_sampled |> 
  summarise(
    Total_Heridos = sum(NUMBER.OF.PERSONS.INJURED, na.rm = TRUE),
    Total_Muertos = sum(NUMBER.OF.PERSONS.KILLED, na.rm = TRUE)
  )
...
datos_resumen <- data_sampled |> 
  summarise(
    Total_Heridos = sum(NUMBER.OF.PERSONS.INJURED, na.rm = TRUE),
    Heridos_Pedestrians = sum(NUMBER.OF.PEDESTRIANS.INJURED, na.rm = TRUE),
    Heridos_Cyclists = sum(NUMBER.OF.CYCLIST.INJURED, na.rm = TRUE),
    Heridos_Motorists = sum(NUMBER.OF.MOTORIST.INJURED, na.rm = TRUE),
    Total_Muertos = sum(NUMBER.OF.PERSONS.KILLED, na.rm = TRUE),
    Muertos_Pedestrians = sum(NUMBER.OF.PEDESTRIANS.KILLED, na.rm = TRUE),
    Muertos_Cyclists = sum(NUMBER.OF.CYCLIST.KILLED, na.rm = TRUE),
    Muertos_Motorists = sum(NUMBER.OF.MOTORIST.KILLED, na.rm = TRUE)
  )
...
ggplot(datos_long, aes(x = Tipo, y = Total, fill = Tipo)) +
  ...
  labs(title = "Total de personas heridas y muertas por tipo",
       x = "Tipo de persona", y = "Total") +
  theme_minimal() +
  theme(legend.position = "none")
```
*HAY QUE HACERLO MÁS GRANDE PARA QUE SE VEA BIEN*
...
```{r}
ggpairs(data_sampled, columns = c("NUMBER.OF.PERSONS.KILLED", "NUMBER.OF.PEDESTRIANS.KILLED", "NUMBER.OF.CYCLIST.KILLED", "NUMBER.OF.MOTORIST.KILLED", "NUMBER.OF.PERSONS.INJURED", "NUMBER.OF.PEDESTRIANS.INJURED", "NUMBER.OF.CYCLIST.INJURED", "NUMBER.OF.MOTORIST.INJURED"), 
